...
## 0. Current status (updated 2026-05-28, T129 done -- F04 private messaging mirrors hardened + RLS regression test; 165 files / 1523 tests)

The three-stage deployment model (PROD / DEV / DEMO) is fully formalized. F04 private messaging write functions now surface user-visible errors via an `onError` callback when the backend mirror fails; a backend-free regression test guards the party-or-admin RLS privacy fix. Next: T130 (link admin-initiated F04 threads to the resident's live account).

- **2026-05-28 — T129 (P2) Live activation: F04 private messaging mirrors hardened.** `startThread`, `reply`, `toggleStatus` in `adminChatApi.ts` accept optional `onError` callback; `AdminChatPage` passes `toast.error(t('adminChat.writeFailed'))`. `writeFailed` locale key added (EN + RO). `privateThreadPrivacyRls.test.ts` (backend-free RLS regression, 7 tests) + `adminChatApi.test.ts` (offline store behavior, 20 tests). 165 files / 1523 tests / build / build:pi / build:demo green. Live smoke pending credentials.
- **2026-05-28 — T178 (P2) Documentation pass for the three-stage model.** `DECISIONS.md` + `PI_DEPLOYMENT.md` updated with three-stage rationale, DEV email workflow, new env vars. 163 files / 1500 tests green.
