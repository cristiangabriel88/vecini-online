...
- date: 2026-06-06
- last_task: T256 Per-tenant feature-flag overrides -- featureOverridesLogic.ts (pure merge/set/clear helpers), featureOverridesStore.ts (shared Zustand store, persisted vecini.feature_overrides), featureStore.ts useFeature() updated to apply overrides, featureApi.ts hydrateFeatureOverrides(), feature-override.ts Netlify function (POST, service-role, is_super_admin re-check, upsert/delete + audit), feature overrides section in PlatformAsociatieDetailPage, audit actions feature.override_enabled/disabled, migration asociatie_feature_overrides, i18n RO/EN, 21 unit tests
- pipeline: green (lint + typecheck + test + build + build:pi + build:demo)
- counts: 298 test files / 2848 tests
...
- next: T244 [P2] Shared per-asociatie store factory
