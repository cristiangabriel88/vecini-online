...
- **Description:** Repository of official documents: statutul asociației, regulamentul de ordine interioară, contracte cu furnizorii (apa, gaz, salubritate), documente cadastrale. Searchable. Role-gated real file upload (T88 done): admin/presedinte/comitet can upload PDF/image/Word/Excel files (max 10 MB); all members see a download button on cards that have a file; delete with confirmation is also gated; upload/delete recorded in the audit trail. Offline: files stored as base64 data URLs in the persisted `vecini.documents` Zustand store. Live Storage path is T89.
- **Acceptance:** Upload (role-gated), categorize, search, version history, download, delete.
- **Telegram:** `/documente` shows categories.
- **Data:** `documents` (+ `file_name/file_size/file_type/file_data_url`; `storage_path` -> Supabase Storage via T89)
...
| F02 | Canal de discuții moderat | ✅ | Moderated discussion channel: topic-tagged threads (pinned float first, then most-recent-activity), expand to read/post messages, comitet pin/unpin and delete-message; validation/sort/last-activity/rate-limit logic unit-tested; `/discutii` bot command. Threads + messages scoped per asociație (T48: `byAsociatie` store, `useAsociatieThreads`, posts attributed to the active author). Tables `discussion_threads/messages`, `moderation_actions` + RLS. |
| F03 | Alertă de bloc (urgență) | ✅ | Send flow with double-confirm bypassing quiet hours; recipient count. |
| F04 | Mesagerie privată cu administratorul | ✅ | Private resident↔admin channel: start a subject-titled thread, chat-style timeline (own messages right / admin left), reply, unread-from-admin badge, an "awaiting reply for Nh" SLA hint, mark resolved/reopen, open-threads-first ordering, and read-on-open receipts; subject/message validation, last-activity, awaiting-reply, waiting-hours, unread-count, sort and status-toggle logic unit-tested; `/contact_admin` bot command. Tables `private_threads/messages` + RLS. |
...
| F33 | Document arhivă | ✅ | Searchable document repository (statut/regulament/contracte/cadastru) with category filter, version display and add; real file upload (T88): admin/presedinte/comitet upload PDF/image/Word/Excel (max 10 MB, base64 data-URL stored offline in persisted store), all members download, delete with confirm modal; upload/delete audit events; category + diacritic-insensitive search + file helpers unit-tested; `/documente` bot command. Live Supabase Storage path is T89. Table `documents` + GIN full-text search + RLS. |
...
| F39 | Wiki bloc | ✅ | Collaborative local-knowledge wiki: add a page (title/body, auto-slug), accent-insensitive search over title/body, newest-updated-first list and Markdown-lite rendering; validation/slugify/search/sort logic unit-tested; `/wiki` bot command. Tables `wiki_pages/revisions/suggested_edits` + GIN search + RLS. |
...
| F49 | Cod portari / vecini de încredere | ✅ | Private owner-only safety profile: set a scam-defeating passphrase + free-text instructions, and keep a name-sorted list of trusted contacts (relationship + tap-to-call), add/remove; a privacy banner stresses it's encrypted and never shared. Passphrase/phone/contact validation, phone-digit normalization, tel-href and diacritic-insensitive sort logic unit-tested; `/cod_siguranta` bot command. Table `safety_codes` (owner-only RLS, encrypted payload). |
| F50 | Plan de evacuare | ✅ | Evacuation plan per scara: text route + safety-fixture list (stingător/hidrant/ieșire/tablou electric, each with location and icon) sorted by stairwell, plus pet markers — residents flag animals in their apartment with a building-wide count so firefighters know where to look (set/clear own). No binary floor-plan ships; route text carries the plan. Plan/equipment/pet-marker validation, stairwell sort, fixture count-by-kind and distinct-pet-apartment logic unit-tested; `/evacuare` bot command. Tables `evacuation_plans`, `pet_markers` + RLS. |
...
| F55 | Sistem alarmă (status) | ✅ | Alarm/detection-system status dashboard with armat/dezarmat/declanșat/test tone badges, days-since-last-test with a 90-day test-due flag and attention banner, add system and log test (rolls last-test forward); status-tone/test-interval/days-since/attention/sort logic unit-tested; `/alarma` bot command. Tables `alarm_systems`, `alarm_events` + RLS. |
...
| F59 | Babysitting / pet-sitting | ✅ | Opt-in sitter profile (kind/availability/rate, save/edit/leave) + searchable, kind-filterable board sorted by name; validation + kind-filter + diacritic-insensitive search logic unit-tested; `/babysit` & `/petsit` bot commands. Table `sitter_profiles` + owner RLS. |
| F60 | Skill exchange / barter | ✅ | Opt-in offering of skills (offers/needs, save/edit/leave) + searchable list sorted by name with offers/needs badges; validation + diacritic-insensitive search logic unit-tested; `/barter` bot command. Table `skill_offerings` + owner RLS. |
| F61 | Grupuri de cumpărături comune | ✅ | Create a group buy (title/details/deadline) and join with one tap (signup tally, join-once guard); active buys soonest-deadline-first, closed ones faded below; validation + open/active/closed-by-deadline logic unit-tested; `/bulk` bot command. Tables `group_buys`, `group_buy_signups` + owner RLS. |
| F62 | Welcome kit for new residents | ✅ | New-resident onboarding checklist: ordered steps the resident ticks off, a live progress bar with done/total + percent and an "all done" celebration, comitet add/delete steps; validation/sort/next-order/completion/is-complete logic unit-tested; `/bun_venit` bot command. Table `welcome_kit_templates` + RLS. |
...
| F66 | Profil complet | ✅ | Rich full-page profile editor at `/app/profil`: photo (center-cropped square + initials fallback, capped data URL offline), structured standard fields (full/display name, phone, email, apartament/scara/etaj, car plate→F28, address, emergency contact, DOB→F63, language) with per-type validation (plate/phone/email/date) and a live completeness indicator; autosaves every change. User-added custom fields via `+ Adaugă câmp` (label + typed catalog text/longtext/number/phone/email/date/bool/select/link/address) each marked private vs. visible-to-neighbours (→F36), with the right input control per type, reorder (up/down) + delete. Account card links to Notificări/Securitate/Datele mele/Confidențialitate + sign-out. Pure model/validation/completeness/custom-field ops + avatar crop maths unit-tested; migration extends `users` + new owner-RLS `profile_custom_fields`; `/profil` bot help; one E2E. Live persistence + Storage avatar bucket + admin/F36 read paths are follow-ups. |
| F67 | Acasă personalizabil | ✅ | Customizable home at `/app`: a `Personalizează` pencil flips the card grid into edit mode where each card has show/hide, up/down reorder + native drag-and-drop, and a compact↔expanded size toggle; every edit autosaves per resident. The card catalog is exactly the asociație's admin-enabled, routed features in registry order (a disabled feature is never offered; `reconcileLayout` drops disabled keys and appends newly enabled ones); the default shows the first six and hides the rest. `Resetează la implicit` restores the default (disabled when already default); an all-hidden empty state guides recovery. Pure layout model/ops (catalog, default split, reconcile, toggle/size/move/move-to, isDefault) unit-tested; persisted `homeLayoutStore` keyed by resident+asociație; migration adds the owner-RLS, tenant-tightened `home_layouts` table; one E2E (hide a card → leaves the grid → survives reload); no bot command (web/Mini App surface). Live persistence under owner RLS is a follow-up (T106). |
