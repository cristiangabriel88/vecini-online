...
- modified: src/features/auth/SecurityPage.tsx, src/features/legal/PrivacySettingsPage.tsx

### T268 ✅ 2026-06-06 -- Onboarding progress indicator

Extracted pure step logic (`clampStep`, `nextCarouselStep`, `prevCarouselStep`, `isLastCarouselStep`) into `welcomeLogic.ts`; `WelcomeCarousel` now imports them for all navigation and keyboard handling. Added a visible `1 / 3` step counter (`<span class="welcome-step-counter">`) rendered inline after the dot tabs using new `welcome.stepCounter` i18n key (bilingual; language-neutral "{{current}} / {{total}}" format). Made the skip button more prominent: border, `font-weight: 500`, `var(--text-secondary)` base with hover bringing in `var(--text-primary)` + `var(--border-strong)`. CSS for `.welcome-step-counter` (xs text, tabular-nums, muted). 14 unit tests in `tests/unit/welcomeCarouselStep.test.ts` covering `clampStep` clamping, `nextCarouselStep` advancement + boundary, `prevCarouselStep` decrement + boundary, `isLastCarouselStep` all cases including single-slide edge. All 313 test files (3055 tests) green, all 3 builds pass.
- modified: src/features/welcome/welcomeLogic.ts, src/features/welcome/WelcomeCarousel.tsx, src/styles/welcome.css, src/shared/locales/ro.json, src/shared/locales/en.json
- new: tests/unit/welcomeCarouselStep.test.ts

### T267 ✅ 2026-06-06 -- Unsaved-changes guard

New `src/shared/lib/useUnsavedGuard.ts`: exports `isFormDirty<T>(current, initial)` (JSON-based pure equality, unit-tested in isolation) and `useUnsavedGuard(isDirty)` which wires React Router's `useBlocker` (data-router only) to block in-app navigation and a `beforeunload` listener to block browser-level exits. A `bypassRef` is set via `clearDirty()` before the post-save `navigate()` call so successful saves are never intercepted. New `src/shared/components/UnsavedChangesModal.tsx` renders the bilingual premium-feel confirm dialog (uses existing `Modal` + `Button` primitives). Adopted in `ApartmentFormPage`: `initialInputRef` and `initialPersonsRef` capture mount-time state; `isDirty` recomputes every render; `clearDirty()` called inside `onSave` before `navigate`. Bilingual keys added to `common.unsavedChanges.*` in both `ro.json` and `en.json` (title, body, stay, leave). 16 unit tests in `tests/unit/useUnsavedGuard.test.ts` covering `isFormDirty` across objects, arrays, primitives, nested structures, and edge cases. 3 E2E scenarios in `tests/e2e/unsavedChanges.spec.ts`: stay on page, confirm leave, clean form passes through. All 312 test files (3045 tests) green, all 3 builds pass.
- new: src/shared/lib/useUnsavedGuard.ts, src/shared/components/UnsavedChangesModal.tsx, tests/unit/useUnsavedGuard.test.ts, tests/e2e/unsavedChanges.spec.ts
- modified: src/features/admin/ApartmentFormPage.tsx, src/shared/locales/ro.json, src/shared/locales/en.json
