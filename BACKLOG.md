...
**Done:** the `invite-email` Netlify function is no longer an open relay. New `netlify/functions/_shared/supabaseAdmin.ts` (service-role client, `verifyBearerToken`, `isAdminOfAsociatie`, `getInviteById`, `getAsociatieName`) and `netlify/functions/_shared/rateLimiter.ts` (pure sliding-window `checkSlidingWindow` + production `checkInviteRateLimit`). Updated function: requires `Authorization: Bearer <access_token>` header; resolves caller via `auth.getUser()` (never trusts a client-supplied user id); looks up the invite row by `inviteId` UUID from the DB (recipient address, name, and token resolved server-side, not from the request body); rejects consumed/revoked/expired invites; rate-limits at 20 sends per 10 min per `userId:asociatieId` pair; asserts caller is admin/presedinte of the invite's asociație; builds the link from `APP_URL` + the DB-stored token. New migration `20260526000005_invite_token.sql` adds `token` column to `invite_codes` (partial unique index). Updated `inviteEmailApi.ts`: live path now sends `{ inviteId, locale }` with `Authorization: Bearer` header (no longer sends recipient or link); offline path unchanged. Cleaned up unused `asociatieName`/`baseUrl` from callers (`ApartmentFormPage`, `InvitesAdminPage`). New `tests/unit/inviteEmailAuth.test.ts` (schema guard for token column + 5 sliding-window unit tests). Commands run: `npm run lint`, `npm run typecheck`, `npm test` (133 files / 1039 tests green), `npm run build`; all green.

### ✅ T153 — [P1] Admin invitation email: polished bilingual HTML template + embedded QR code

**Done:** `invite-email` Netlify function now has two distinct templates selected by a `kind` field in the request body. New `buildAdminInviteEmail` exported from `src/shared/lib/inviteEmail.ts` (`AdminInviteEmailParams`, `AdminInviteEmailParams`): branded header bar (blue accent, "vecini.online" / "Platforma asociatiilor de proprietari"), greeting by name with fallback, bilingual admin-role intro ("Ati fost invitat ca Administrator pe platforma vecini.online" / "You were invited as an Administrator on the vecini.online platform."), a large rounded-pill CTA button (`border-radius: 9999px`, drop shadow), an optional QR code block (`<img src="data:image/png;base64,...">` 200x200 with scan hint), fallback copyable link, and a footer with 24h expiry notice + ignore text. The template is pure and dependency-free (the `qrCodeDataUrl` is passed in as a pre-generated string). In `invite-email.ts`: added `kind?: 'admin_setup' | 'resident_invite'` to `InviteEmailRequest`; when `kind === 'admin_setup'` the function calls `QRCode.toDataURL(link, { width: 200, margin: 2, color: ... })` (non-fatal catch: omits QR block on failure) then `buildAdminInviteEmail`; otherwise falls through to the existing `buildInviteEmail`. Added `qrcode` to `dependencies`, `@types/qrcode` to `devDependencies`. 8 new unit tests in `inviteEmail.test.ts`: bilingual subject/CTA, neutral-greeting fallback, QR embedding, QR omission, HTML escaping of admin name, 24h expiry + ignore notice, branded header + pill button. Commands run: `npm run lint`, `npm run typecheck`, `npm test` (133 files / 1047 tests green), `npm run build`; all green.

### ✅ T152 — [P1] Superadmin: dedicated "Add Asociatie" page (admin name + email + send email)

**Done:** see COMPLETED.md.
...
**Done:** new `supabase/migrations/20260527000001_mfa_channels_hook.sql` adds three tables and the Custom Access Token Hook. `mfa_channels(user_id, channel, enabled_at, target_hint)`: RLS on; self read/insert/delete policies scoped to `user_id = auth.uid()`; unique `(user_id, channel)`; channel check `('email','telegram')`. `mfa_otp_challenges(id, user_id, channel, code_hash, code_salt, confirm_token_hash, created_at, expires_at, consumed_at, attempts, session_id)`: RLS on; zero client policies (service-role-only deny-all). `session_elevations(id, user_id, session_id unique, channel, elevated_at, expires_at)`: RLS on; zero client policies; channel check includes `'recovery'` for T29. `custom_access_token_hook(event jsonb)`: SECURITY DEFINER, `set search_path = ''` (injection guard); reads `event->'claims'->>'session_id'`, looks up the freshest non-expired `session_elevations` row, merges `app_2fa_at` (epoch) + `app_2fa_channel` into claims; returns event unchanged when no elevation found; `revoke all from public` + `grant execute to supabase_auth_admin` only. Migration header documents the live-activation step (dashboard: Authentication > Hooks). All migration content is inert until the hook is registered. New `tests/unit/mfaChannelsHook.test.ts` (24 tests): schema-parse guards for all three tables (create, RLS-on, policy counts), security properties of the hook (SECURITY DEFINER, `search_path = ''`, revoke-from-public, grant-to-supabase_auth_admin, NOT granted to authenticated/anon), functional shape (app_2fa_at + app_2fa_channel injection, safe pass-through). Commands: `npm run lint`, `npm run typecheck`, `npm test` (137 files / 1141 tests), `npm run build`; all green.
...
**Done:** `generateApartmentsCsvTemplate(): string` added to `src/shared/lib/csv.ts` (CRLF-delimited, header `scara,numar_apartament,name,email,numar_persoane,proprietar,opt_in`, 3 realistic Romanian sample rows). `ApartmentsPage`: imported `Download` icon + new `handleDownloadTemplate` handler (Blob + `<a download="sablon-apartamente.csv">`); "Descarca sablon .csv" / "Download .csv template" button added to both the populated header action bar and the empty-state action area. New locale keys `apartments.downloadTemplate` in `ro.json` + `en.json`. 5 new unit tests for the template generator in `tests/unit/csv.test.ts` (non-empty, header content, 4-line count including header, sample row values, CRLF endings). Commands run: `npm run lint`, `npm run typecheck`, `npm test` (134 files / 1072 tests green), `npm run build`; all green.
...
Surfaced in T140: the `SecurityPage` step-up card (`needsStepUp` path, T112) still shows only the TOTP/recovery-code input. Now that email and Telegram channels exist, the step-up should offer the same channel picker and OTP flow as the login page — if the session has email or Telegram enabled (and no TOTP enrolled, or the user prefers not to use TOTP), they should be able to satisfy the AAL2/app2fa gate from the security page using an OTP channel. Mirror the LoginPage OTP flow (channel picker, requestOtp, demo code affordance, verifyOtp) in the step-up section of SecurityPage, setting `app2faSatisfied` via the same path as the login flow. Prereq: T140; coordinates with T112, T143 (live session elevation).
