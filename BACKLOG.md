...
### ⬜ T166 — [P3] Skip the feature-request clear when a toggled-on module has no pending demand
Surfaced in T161: the per-feature `Switch` on `FeaturesAdminPage` now calls `clearFor(asociatieId, key)` on every off->on enable so a satisfied request is cleared regardless of which control enabled the module, but most modules an admin turns on have no pending request, so live this fires a no-op `mirrorClear` DB delete (the "admin clear asociatie feature requests" policy) on nearly every enable. Guard the call so it only fires when the local store actually holds a request for that `(asociatie, module)` (e.g. check `summarizeRequests`/a cheap `hasAnyRequest` selector before clearing), avoiding the wasted round-trip while keeping the clear behaviour identical when there is demand. Trivial, backend-free locally; trims a live DB call. Prereq: T161.
