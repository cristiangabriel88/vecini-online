...
### ⬜ T124 — [P1] Account-creation-on-redemption: a "set your password twice" landing page
Surfaced reviewing the onboarding data flow (D2, Stages B + D): `JoinAsociatiePage` only takes a code and assumes an already-authenticated user, with no password step. Add a shared bilingual "create your account" landing page (reached by the setup/invite link or by the fallback code) that lets the invitee enter their email and **set a password twice** (reusing `src/features/auth/passwordPolicy.ts`), then on submit consumes the token/code (single-use, 24h, replay-safe) and activates the membership offline — `admin` for a setup link, the code's role + apartment link for a locatar invite — landing them in `/app`. Add the setup route (e.g. `/configurare-cont`) and supersede the code-only entry in `JoinAsociatiePage.tsx`. Unit test the redemption logic + one E2E happy-path (open link, set password, land in `/app`; expired/reused link rejected). Offline-first. Prereq: T123. Live account creation on redemption is T55. **From T123:** the landing reads the `?token=` query param and redeems via `inviteStore.consumeByToken` (the code path stays the manual fallback); move `ONBOARDING_REDEEM_PATH` in `src/shared/lib/inviteCode.ts` to the new `/configurare-cont` route so the already-built links resolve to it.
