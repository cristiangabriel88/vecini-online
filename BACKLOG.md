...
Done: Created `netlify/functions/audit-hmac.ts` (POST-only, bearer auth, per-IP 60 req/60s, validates 16-char hex tail_hash, re-checks caller is admin of the given asociatie via `isAdminOfAsociatie`, signs `v1:{asociatieId}:{tailHash}` with HMAC-SHA256 keyed on `AUDIT_HMAC_SECRET`; returns `{hmac, algorithm:'hmac-sha256'}` when secret is set, `{hmac:null, configured:false}` otherwise; documents activation steps and recommended external anchoring as the follow-up). Added pure `hmacCanonical(asociatieId, tailHash)` export to `auditLogic.ts` (versioned format `v1:...`). Extended `auditStore.ts` with `chainHmacByAsociatie` state field (ephemeral, not persisted) and `fetchChainHmac(asociatieId)` action (gets session token, calls the endpoint, stores result). Updated `AuditLogPage.tsx` to call `fetchChainHmac` on mount and show a `Lock` + "HMAC-signed chain" / "Lanț semnat HMAC" badge when a non-null HMAC is returned. Added `audit.hmacSigned` locale key to ro.json and en.json. Created `tests/unit/auditHmac.test.ts` with 6 assertions. 287 files / 2743 tests / lint + typecheck + build + build:pi + build:demo all green.
...
Done: Created `tests/unit/rlsPolicyCoverage.test.ts` with 5 assertions. The guard parses all migration SQL and builds two sets: `enabled` (tables with RLS on) and `hasPolicies` (tables with at least one `create policy ... on TABLE` or macro call). A named `INTENTIONAL_DENY_ALL` map documents 4 service-role-only tables that deliberately carry zero client-facing policies: `mfa_otp_challenges`, `session_elevations`, `login_attempt_locks`, `mfa_recovery_attempt_counts` (all accessed exclusively through SECURITY DEFINER functions or the service-role client). The core assertion checks every zero-policy table is in the map; two inverse assertions flag stale entries and entries that no longer have zero policies, keeping the map self-maintaining. 288 files / 2748 tests / lint + typecheck + build + build:pi + build:demo all green.
