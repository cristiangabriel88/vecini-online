...
### ⬜ T161 — [P3] Clear satisfied feature requests when a module is enabled via the normal toggle
Surfaced in T151: the triage "enable" action flips the flag *and* clears the module's pending requests, but enabling the same module through the regular per-feature `Switch` on `FeaturesAdminPage` only flips the flag, leaving the request rows behind (the triage list filters them out via `!flags[key]`, so they are invisible there, but offline they linger in `featureRequestStore` and live they linger in `feature_requests`). When a flag transitions off->on through the normal toggle, also call `clearFor(asociatieId, key)` so the satisfied demand is cleared the same way regardless of which control enabled the module. Pure clear logic already unit-tested; add the toggle-path guard. Backend-free offline; the live delete mirror already exists. Prereq: T151.
...
Surfaced in T131: the shared `Input` component renders its inline error with an `id={fieldId}-err` and points the control at it via `aria-describedby` (and `aria-invalid`), but the sibling `Textarea` in the same file sets `aria-invalid` only and renders the error paragraph without an `id`/`aria-describedby` pair, so a screen-reader user editing a textarea is told the field is invalid but never hears the error text. Mirror the `Input` pattern on `Textarea`: give the error `<p>` an `id={fieldId}-err` and set `aria-describedby` on the `<textarea>` when an error is present. Trivial, backend-free; no behaviour change for sighted users. Prereq: none.
