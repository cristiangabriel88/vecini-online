...
Confirmed blocker for the live flow (surfaced planning the prod MVP). `sendInviteEmail` (`src/features/invites/inviteEmailApi.ts:57`) POSTs `inviteId: input.invite.id`, but local invite ids carry the `inv-` prefix (`inv-<uuid>`) while the Netlify `invite-email` function rejects any id that is not a bare UUID (`netlify/functions/invite-email.ts:84` returns 400 `invalid-invite-id`). So every live resident-invite email fails today. Strip the prefix in `sendInviteEmail` before posting (mirror `writeInviteToLive`'s `id.startsWith('inv-') ? slice(4) : id` at `src/features/invites/inviteWriteApi.ts:36`) so the posted id matches the row `writeInviteToLive` inserted, and add a unit test asserting the posted `inviteId` is the bare UUID. The superadmin admin-invite path is unaffected (provision-asociatie mints + sends server-side). Prereq: T55.
