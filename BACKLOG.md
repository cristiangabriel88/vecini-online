...
### ✅ T296 — [P1] E2E for self-service 2FA recovery + email-method enable (T295 follow-up)

T295 shipped self-service lost-authenticator recovery and email-code 2FA as a standalone, user-selectable factor, with unit coverage but no Playwright spec. Add one demo-mode E2E (deterministic, frozen time/animations): enrol TOTP in Security, sign out, sign in with password, open the "Lost access to your authenticator?" disclosure on the challenge, request the account-email code (demo shows it on-screen), enter it, reach the app, then use "Reset authenticator" to start a fresh enrolment. A second flow: enable the **email** method via the setup-confirm step in Security, sign out, and complete a login challenge using the email code as the only factor. Reuse the demo OTP affordance already rendered on the challenge/security screens. Prereq: none.

### ✅ T297 — [P2] Platform-admin "reset a user's 2FA" support action (total-lockout fallback)

T295's email recovery covers the common case (lost authenticator, still has the account inbox), but a user who loses BOTH the authenticator and access to their account email is still stranded and needs manual `auth.mfa_factors` surgery. Add a service-role Netlify function + a Platform console action (gated by `is_super_admin()`, re-checked server-side, audited into the tamper-evident chain via `appendAudit`) that clears a target user's TOTP factors, `mfa_channels`, `mfa_recovery_codes`, and `session_elevations` by email, so the user falls back to password-only and re-enrols. Bilingual confirmation UI; never expose the service-role key; rate-limited. Unit/handler tests asserting it rejects unauthenticated and non-platform-admin callers. Prereq: none.
