...
### ⬜ T168 — [MVP] Live environment + Resend email provider setup
Stand up the live runtime so the flow is reachable end-to-end. Confirm + document every env var the live path reads: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (functions only, never client), `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `APP_URL`, `VITE_APP_URL`, `VITE_RESIDENT_APP_URL` — in `.env.example`, `netlify.toml`, `netlify-platform.toml`. Write a short `RUNBOOK-MVP.md` capturing the manual steps: create a Resend account, verify a sending domain (recommend `vecini.online`, set `RESEND_FROM_EMAIL=noreply@vecini.online`), set the env vars on both Netlify sites, and the Supabase auth config (Site URL = resident origin, add the platform origin, **disable "Confirm email"** so the invite link itself is the verification -> one email then set password). Verify `isResendConfigured()` / `isSupabaseAdminConfigured()` / `isSupabaseConfigured` gate correctly and the invite link base URL resolves from `APP_URL`. Record the email-confirmation + run-target (deploy to Netlify) decisions in `DECISIONS.md`. Config + runbook only; no flow logic. Prereq: none.

### ⬜ T169 — [MVP] Live superadmin account + `is_super_admin()` grant verified
So the superadmin can actually log into the platform console live. Ensure a real superadmin auth user exists and `is_super_admin()` returns true for it: add a seed/grant migration under `supabase/migrations/` (applied by the Hermes migration agent) that registers the superadmin in `platform_admins` and sets an explicit least-privilege `grant execute on function is_super_admin() to authenticated` (revoke from `anon`/`public`). Confirm `platformAuthStore.verify()` -> `supabase.rpc('is_super_admin')` resolves under a real session and `RequirePlatformAdmin` grants/denies correctly. Add a backend-free guard parsing the migration for the explicit grant so the gate's contract cannot silently regress. Folds the substance of T118. Prereq: T91, T93, T168.

### ⬜ T92 — [MVP] Server-side privileged provisioning (Netlify function, service role) + wire the superadmin live send
Today the superadmin "Trimite invitatie" only writes a local Zustand record and simulates the email (`PlatformAddAsociatiePage.tsx` `handleSubmit`). Add a Netlify function (e.g. `netlify/functions/provision-asociatie.ts`) using `_shared/supabaseAdmin.ts` (service role) that (1) **re-verifies the caller is `super_admin` server-side** via `is_super_admin()`, (2) creates the asociatie, (3) issues a 24h admin setup invite **row** the email step can resolve, and (4) dispatches the real `kind: 'admin_setup'` email (reuse `buildAdminInviteEmail` / the `invite-email` function path). Reuse the pure `platformProvisioningLogic.ts`. Wire the `PlatformAddAsociatiePage` live branch to call this function instead of `markAdminEmailSent`; keep the offline/demo path as the fallback. Account-on-redemption stays: do **not** pre-create the admin's auth user here (the admin sets their password at redemption, T55). Prereq: T169, T168.
