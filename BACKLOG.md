...
The next wave of work drives a single, real, usable SaaS loop rather than more breadth or more compliance surface. The whole loop must work **in DEMO stage (frontend-only, `VITE_APP_STAGE=demo`)** with no backend provisioned — that is the version overnight work builds and verifies. DEV stage (Pi + local Supabase) and PROD stage (cloud) layer on top as separate, smaller activation steps. A direct production blocker (red pipeline, active security/data-loss hole) always outranks everything.

**Acceptance criteria (the loop — DEMO stage first, live-ready):**
...
Done: `adminChatApi.ts` hardened -- `startThread`, `reply`, and `toggleStatus` now accept an optional `onError?: () => void` callback that is called when the Supabase mirror fails (checking the `error` field of each insert/update rather than swallowing it silently). `AdminChatPage.tsx` passes `onSyncError = () => toast.error(t('adminChat.saveFailed'))` to all three write paths. `markRead` stays silent (low-stakes). Locale key `adminChat.saveFailed` added to both `ro.json` and `en.json`. 16 new tests in `adminChatApi.test.ts` covering offline-path contracts (hydrateThreads no-op, startThread/reply/markRead/toggleStatus store updates, onError never called without a backend). Live activation steps (apply migration `20260525000002_private_threads_inbox.sql`, verify party-or-admin RLS with resident + admin JWTs, add live E2E proving cross-thread isolation) are deferred until a provisioned Supabase project is available. 162 files / 1504 tests / build / build:pi / build:demo green.
