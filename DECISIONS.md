...
- choice: (1) the admin provisioning functions (`provision-asociatie`, `provision-additional-admin`) now store `sha256(token)` in `invite_codes.token` (the onboarding RPCs from migration 20260528000003 hash before lookup, so plaintext-at-rest rows could never resolve -- every fresh admin setup link was dead in production); `admin-invite-action` resolves a UUID-shaped inviteId by `id` and any other value by hashing it and matching the digest, replacing the `.or(token.eq.X,id.eq.X)` filter that both interpolated client input and threw a uuid cast error for token-shaped values. (2) Live platform flows adopt the server-returned `invite_codes` UUID as the local mirror id (`inviteAdmin(name, email, id?)` / `provisionAdditionalAdmin(..., inviteId?)` + `AdminProvisionRecord.inviteId`) so resend/revoke address the real DB row instead of a locally minted UUID that 404s. (3) `adminChatStore` mints `genId()` (uuid-format) ids instead of `pt-${Date.now()}`/`pm-${Date.now()}` -- the `private_threads`/`private_messages` id columns are `uuid`, so every new live chat insert failed the cast. (4) All fire-and-forget comunicare mirrors (announcements delete, discussion pin/delete/edit, chat start/reply/read/delete/toggle) now check the supabase `{ error }` result (supabase-js does not throw on RLS/PostgREST failures) and roll the optimistic store change back via a pre-change snapshot. (5) Discussions (threads + messages, including soft-delete propagation) joined the Realtime subscription set; hydrates gained newest-first caps (200 announcements / 100 threads). (6) No SQL migration: existing plaintext-token rows were already digested by migration 20260528000003's one-time UPDATE; only rows written by the broken functions after that migration are unredeemable -- they expire in 24h by design and a resend now re-mints them correctly.
- why: the audit was explicitly production-first ("auth, invitations, comunicare must work flawlessly in production"); all six were silent production-only failures invisible in demo mode, where the local stores are authoritative and no DB write happens.
- alternatives rejected: (a) reverting the RPCs to plaintext lookup -- weakens T128's hash-at-rest guarantee for a writer-side bug; (b) inverse-operation rollbacks everywhere -- snapshot/restore is simpler and the realtime-overwrite window is negligible at this write volume; (c) repairing in-flight plaintext invite rows via migration -- impossible (the plaintext cannot be recovered from the row; the affected invites are 24h-expired anyway).
- blast radius: `netlify/functions/{provision-asociatie,provision-additional-admin,admin-invite-action}.ts`, `src/platform/{platformAsociatiiStore,PlatformAddAsociatiePage,PlatformAsociatieDetailPage}.tsx?`, `src/features/adminchat/{adminChatStore,adminChatApi}.ts`, `src/features/discussions/discussionApi.ts`, `src/features/announcements/announcementsApi.ts`, `src/app/{realtimeLogic,useRealtimeSync}.ts`, new tests `inviteTokenAtRest.test.ts` + `realtimeDiscussions.test.ts`, docs (`DEPLOYMENT.md` Steps 1+4-7 rewritten to the real invite-only flow, `SECURITY.md` known-gaps refresh, `ONBOARDING_FLOW.md` demo/live table), BACKLOG follow-ups T298-T302.
...
- choice: `vite-plugin-pwa` v1.x in `generateSW` mode with `registerType: 'prompt'` and `injectRegister: null`. PROD and DEMO stages get a full Workbox precache SW (221 entries, ~2.8 MB); DEV (Pi) stage sets `selfDestroying: true` so the plugin generates a minimal SW that unregisters itself and clears all caches on activate -- protecting rapid Pi iteration from stale caches. The plugin is always included in the Vite config so `virtual:pwa-register/react` resolves in every build without conditional imports. `devOptions.enabled: false` keeps the Vite dev server fast. `injectRegister: null` prevents the plugin from injecting a `<script>` into both `index.html` and `platform.html` (the multi-page build input); instead `useRegisterSW` in `<UpdatePrompt>` handles registration via the React hook. The manifest is served unchanged from `public/manifest.webmanifest` (manifest: false). The SW `navigateFallbackDenylist` excludes `/platform/*`, `/.netlify/*`, and `*.map` files. The existing CSP `worker-src 'self' blob:` already covers the SW script.
- why: `manifest.webmanifest` existed but no SW was generated, so the app was "installable in name only" with no offline shell and no update prompt. Adding a SW completes the PWA story: the app shell is cached after first load, navigation works offline, and users get a prompt when a new build ships.
- alternatives rejected: (a) hand-rolled SW -- more code, same outcome; workbox handles precache versioning and cleanup correctly; (b) enable SW for DEV stage -- Pi iterates rapidly and stale caches would cause confusing behaviour where new code isn't picked up; (c) selfDestroying only for DEV, skip plugin entirely -- would make `virtual:pwa-register/react` unavailable in DEV builds, requiring conditional dynamic import in `UpdatePrompt`.
- blast radius: `vite.config.ts` (VitePWA plugin import + config), `src/vite-env.d.ts` (pwa-register type reference), `src/shared/components/UpdatePrompt.tsx` (new), `src/app/providers.tsx` (UpdatePrompt wired in), `src/styles/shell.css` (.update-prompt CSS), `src/shared/locales/ro.json` + `en.json` (pwa.* keys), `tests/unit/swRegistration.test.ts` (new, 9 tests). New devDep: `vite-plugin-pwa@1.3.0`.

### T258b: source-map symbolication architecture
- choice: emit hidden source maps (`sourcemap: 'hidden'` via Vite's function-form config with `loadEnv` to read mode-specific env at config time) for PROD and DEV builds; DEMO build skips maps entirely (no Netlify functions). Maps are blocked from public CDN access via a `[[redirects]] 404 force=true` rule in `netlify.toml`. A post-build upload script (`scripts/upload-sourcemaps.mjs`, wired as `npm run upload-sourcemaps`) pushes maps to a private Supabase Storage bucket `source-maps/<release>/`. Symbolication is on-demand: the `/consola/erori` platform console has a "Symbolicate" button per error group that calls `/.netlify/functions/symbolicate-stack` (service-role, re-checks `is_super_admin()`). The function fetches the matching `.js.map` from Storage by `<release>/<chunk-filename>.map` and resolves frames using `source-map-js` (pure JS, no WASM, compatible with Netlify Node.js lambdas). Stack traces are stored in the `platform_error_reports.stack` column (new migration T258b). The `platformErrorStore` now includes stack in `PlatformErrorReport` and `ErrorGroup`. The pure `sourcemapUtils.ts` helper (with full unit test coverage) handles frame parsing for both Chrome and Firefox/Safari stack formats.
- why: stack traces in `platform_error_reports` were pointing at minified bundle positions (e.g. `main-abc123.js:1:82345`) with no way to map back to source. This made the error console useful for counting/grouping but not for debugging. Storing the stack and adding on-demand symbolication closes that gap without adding a third-party service.
- alternatives rejected: (a) always-on symbolication at report time -- requires source maps available at the error-report function layer, adds latency to every error flush; (b) client-side symbolication with manual `.map` file upload -- acceptable UX but maps are transient (cleared on page refresh) and requires the operator to download the map from the build artifact manually; (c) Sentry/GlitchTip integration -- introduces a third-party data processor which violates the in-house-only constraint in DECISIONS.md.
- blast radius: `vite.config.ts` (function form, `loadEnv`, `sourcemap`), `netlify.toml` (new `.map` 404 redirect), `netlify/functions/symbolicate-stack.ts` (new), `netlify/functions/error-report.ts` (stack field + 16 KB body limit), `src/platform/platformErrorStore.ts` (type + `ErrorGroup` stack/latestRelease fields + demo stacks), `src/platform/platformApi.ts` (select + rowToErrorReport), `src/platform/PlatformErrorsPage.tsx` (stack expand + symbolicate UI), `src/shared/lib/sourcemapUtils.ts` (new pure helpers), `scripts/upload-sourcemaps.mjs` (new), `supabase/migrations/20260606000012_error_reports_stack.sql` (new), locales. New dep: `source-map-js` (bundled with the function by esbuild).
...
- choice: extract each feature page's compose/create form into a self-contained child component (`AnnouncementComposeModal`, and by extension any `*ComposeModal` or `*CreateModal`) that owns all draft state (`title`, `body`, `category`, `schedule`, `pendingFiles`, `saving`, etc.). The page retains only the `open` boolean and stable `useCallback` handlers. Each list item row is extracted as a `React.memo`-wrapped component. The `sanitizeHtml` / DOMPurify call inside each row is memoized with `useMemo` keyed on `body_html`. The visible-items derivation at page level is wrapped in `useMemo([canManage, all])`.
- why: previously the draft fields lived at page level, so every keystroke re-rendered the entire page including the list of items, calling `sanitizeHtml(body_html)` once per item per character. On a 20-item list with DOMPurify's full DOM-parse, this caused measurable lag. The fix eliminates the re-render chain: keystrokes stay inside the modal subtree; `React.memo` + stable `useCallback` props prevent list rows from re-rendering when the parent re-renders for other reasons (open/close, bulk-selection changes).
- alternatives rejected: (a) `React.useRef` for draft fields in the page -- avoids re-renders but defeats controlled inputs and clashes with the Button disabled guard; (b) `useReducer` at page level -- same scope problem, just with a different shape; (c) global draft state in Zustand -- overkill, adds store surface for ephemeral UI state.
- blast radius: `src/features/announcements/AnnouncementsPage.tsx` (refactored -- `AnnouncementComposeModal` + `AnnouncementRow` extracted); `tests/unit/announcementsRenderIsolation.test.tsx` (new, 4 assertions). Pattern documented here for T241 (roll-out to remaining feature pages).
...
- choice: while adding F02/F04/F05 e2e, found the harness fully broken: the app auto-enters demo at `/` (so the login "modul demonstrativ" button the test helpers click no longer exists), and a hard `goto` to any `/app/*` deep link drops the in-memory demo session and bounces to `/app`, defeating every feature test. Fixes: (1) `RequireAuth` re-enters the persisted demo persona in place on a deep-link reload in the demo stage (not just from `/`); (2) `features.spec.ts` `enterDemo` made build-agnostic; (3) extracted `readLastDemoRole`/`DEMO_ROLES` to `src/shared/lib/demoRole.ts` (re-exported from `@/app/router`) to avoid a router<->RequireAuth cycle; (4) hid `.dev-role-switcher--floating` on the mobile layout (it overlapped page content and intercepted pointer events at 412px).
- why: the deep-link bootstrap is aligned with the documented T174 intent ("a hard refresh re-enters the same persona") -- previously that only held at `/`, losing the path. It is gated to `isDemo()`, so the live/prod login flow is unchanged. Hiding the floating dev switcher on mobile is consistent with how the sidebar is dropped there and removes an un-premium overlay; the login-screen inline switcher still lets you pick a persona.
- alternatives rejected: navigating via sidebar clicks instead of `goto` -- impossible on mobile where the sidebar is `display:none`; `force:true` clicks to punch through the overlay -- masks a real layout bug and is flaky; rewriting all five spec files' entry helpers + the login-page specs (auth/consent/isolation/smoke/batch) -- out of T187 scope, that harness rework belongs to T16.
- blast radius: `RequireAuth.tsx`, `router.tsx`, new `demoRole.ts`, `shell.css`, `features.spec.ts`. No live/prod behaviour change. Five pre-existing feature e2e (F07/F18/F35/F36/F40) still fail on stale search selectors -- left for T16. `make task`, so no new tasks were filed for them.
