...
#               the per-task trigger (default "make task") to complete exactly one
#               task (commit + push). "make task" follows the full Definition of
#               Done but SKIPS the "feed the loop" step, so it drains the queue
#               without adding new tasks. The BUILD phase therefore strictly
#               converges: every pass leaves the queue the same size or smaller.
#
...
#               the docs bigger, better, and more polished. Because BUILD now uses
#               "make task" (no queue growth), REPLENISH is the SOLE source of new
#               work -- it is what keeps the self-improving loop fed.
#
...
#   * --no-replenish is set and the queue drains to zero (pure convergence mode),
...
#
# Usage:
#   ./scripts/run-overnight.sh                  # build (make task), then keep improving
#   ./scripts/run-overnight.sh --no-replenish   # drain the queue to zero, then stop
#   ./scripts/run-overnight.sh --max-hours 8    # stop by morning
#   ./scripts/run-overnight.sh --max-tasks 5
#   ./scripts/run-overnight.sh --model opus     # use a stronger model for hard tasks
#
# Options:
...
#                      this to converge: drain the backlog to zero and finish.
#   --model NAME       Pass --model to Claude (e.g. "sonnet", "haiku", "opus").
#                      Default: "sonnet" (cheaper/faster per pass; raise to "opus"
#                      for harder tasks). Pass an empty string to use your
...
#   --prompt TEXT      The build trigger. Default "make task". Use "make progress"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-replenish)  REPLENISH_ENABLED=0; shift ;;
...
# Because the BUILD phase uses "make task" (no queue growth), this pass is the only
# place new tasks enter the queue -- so it is deliberately demanding.
read -r -d '' REPLENISH_PROMPT <<'EOF' || true
The BACKLOG.md task queue is empty (or the last build pass stalled). Do NOT build a
feature this pass. The autonomous build phase runs `make task`, which never adds
tasks, so THIS pass is the only place new work enters the queue. Run a
replenish/audit pass that grows the next wave of work and raises the quality bar:
...
4. Turn every finding and every worthwhile improvement into a NEW task inserted at
   its correct priority position in the BACKLOG.md `## Main queue` section
   (P0 -> P1 -> P2 -> P3), each with the next free T## id, a one-line goal, and a
   [P#] tag. Keep the queue sorted with the highest priority on top. Add real,
   specific work - not filler.
...
# set_stage <message> : record + print the current stage.
set_stage() {
    {
        echo "HB_PASS=${pass:-0}"
...
# broken commit halts the loop instead of stacking more work on top of it. This
# mirrors the project's non-negotiable verification set, including all three stage
# builds (PROD / DEV-Pi / DEMO), so a commit that only breaks build:pi or
...
# Run one Claude pass. Sets globals PASS_COMMITTED (0/1), PASS_OUTPUT (text), and
# PASS_RC (Claude's exit code) so the caller can tell a crash from a clean no-op.
PASS_COMMITTED=0
PASS_OUTPUT=""
PASS_RC=0
invoke_pass() {
    ...
    if [[ "$before" != "$after" ]]; then
        ...
        PASS_COMMITTED=0
        if (( PASS_RC != 0 )); then
            write_both "[$phase] claude exited non-zero (rc=$PASS_RC) with no commit."
        fi
    fi
}
...
while true; do
    ...
    # Convergence mode: with replenish disabled, an empty queue means we are done.
    if (( is_build == 0 )) && (( REPLENISH_ENABLED == 0 )); then
        write_both ""
        write_both "Backlog drained to zero and --no-replenish is set. Convergence reached. Stopping."
        break
    fi
    ...
    # If a build pass stalls (no commit), optionally try one replenish to re-plan /
    # unblock before counting it as a stall. With --no-replenish we skip straight
    # to the stall accounting (no extra work is generated).
    if (( is_build == 1 )) && (( committed == 0 )) && (( REPLENISH_ENABLED == 1 )); then
    ...
    set_stage "Verifying: re-running the full pipeline on the new commit"
