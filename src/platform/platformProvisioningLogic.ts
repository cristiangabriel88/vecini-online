...
/** A validated, trimmed provisioning request. */
...
export type ProvisionFieldError = 'required' | 'tooShort' | 'email';
...
export function blankProvisionInput(): ProvisionInputDraft {
  return { asociatieName: '', city: '', adminName: '', adminEmail: '' };
}

/**
 * Validate and normalise a provisioning draft. Pure: returns the per-field error
 * codes and, when clean, the trimmed request. Minimum lengths guard against an
 * obviously incomplete asociație name (3) / city + admin name (2); the admin
 * email reuses the shared `isValidEmail`.
 */
export function validateProvisionInput(draft: ProvisionInputDraft): ProvisionValidation {
  ...
  const value: ProvisionInput | null =
    Object.keys(errors).length === 0 ? { asociatieName, city, adminName, adminEmail } : null;
  return { errors, value };
}
