...
function sha256Pure(data: Uint8Array): Uint8Array {
  ...
  const bitLen = len * 8;
  // Pad to 512-bit blocks: append 0x80, zeros, then 64-bit big-endian bit length.
 ...
 * Falls back to a pure-JS FIPS 180-4 implementation for non-secure contexts
 * such as the Pi DEV build accessed over plain HTTP on the LAN.
 */
export async function sha256Hex(text: string): Promise<string> {
  ...
  return Array.from(sha256Pure(bytes), (b) => b.toString(16).padStart(2, '0')).join('');
}
